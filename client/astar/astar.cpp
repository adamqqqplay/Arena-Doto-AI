...
{
    unsigned int deltaX = abs(current.x - end.x);
    unsigned int deltaY = abs(current.y - end.y);
    unsigned int h_value = std::max(deltaX, deltaY) + 0.41 * std::min(deltaX, deltaY); //end.distance(current);
    //unsigned int h_value = sqrt((current.x - end.x) * (current.x - end.x) + (current.y - end.y) * (current.y - end.y));
...
std::vector<AStar::Vec2> AStar::find(const Params &param)
{
    ...
    while (!open_list_.empty())
    {
        ...
        // 是否找到终点
        if (current->pos == param.end or current->pos.distance(param.end)<=1 or count>=1000)
        {
